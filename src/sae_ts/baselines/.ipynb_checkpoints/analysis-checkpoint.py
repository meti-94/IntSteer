...
import sys
sys.path.append('/home/unsw.mahdi/SAE-TS/src')
import os
import torch
...
from sae_ts.ft_effects.utils import LinearAdapter, steer_model, my_steer_model
...
def load_sae_model(config):
    ...
    if sae_load_method == 'saelens':
        ...
        sae.load("/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/saes/gemma-2-2b-12.pth", map_location="cpu")
    else:
...
def without_analyse_steer(model, steer, hp, path, method='activation_steering'):
    ...
    base_path = '/mnt/weka/unsw.genaisim/unsw.mahdi/artifacts/data/'
    with open(os.path.join(base_path, f"generated_texts_{method}.json"), 'w') as f:
...
if __name__ == "__main__":
    ...
    if big_model:
        ...
        model = HookedSAETransformer.from_pretrained_no_processing(model_name = model_name, 
                                                                          ...
                                                                          center_unembed=True,
                                                                          device=device)
        # print(model.unembed(), type(model.unembed()))
        # sys.exit()
# %%
if __name__ == "__main__":
    cfgs_dir = "steer_cfgs"

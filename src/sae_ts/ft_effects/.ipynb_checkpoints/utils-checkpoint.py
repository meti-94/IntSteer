...
def steer_model(model, steer, hp, text, scale=5, batch_size=64, n_samples=128):
    ...
    all_gen = []
    n_layers = getattr(getattr(model, "cfg", None), "n_layers", None)
    # Storage for logits and entropy
    storage = {'before':[], 'after':[], 'entropy_last':[], 
               'bf': {L: [] for L in range(n_layers)}, 
               'pattern':{L: [] for L in range(n_layers)}}
    

    hooks = [
        # (f"blocks.13.attn.hook_pattern", partial(patch_pattern, storage=storage, key='pattern', layer=L)), 
        (hp, partial(patch_resid, steering=steer, scale=scale/300, storage=storage)),
        ...
        # ("blocks.12.hook_resid_post", partial(capture_bf, unembed=model.unembed, storage=storage, key='entropy_immediate'))  # last hidden state
    ]
    ...
    temp_patterns = {L:[] for L in range(n_layers)}
    ...
    storage['bf'] = [bf_process(storage['bf'][L]) for L in range(n_layers)]
    # print(storage['bf'][25])
    # print(storage['bf'][0], storage['bf'][0].size())
    # print(entropy_last, entropy_last.size())
    # print('')
    # print(temp_patterns[10].size())
    # sys.exit()
    # entropy_immediate = bf_process(storage['entropy_immediate'])
...
def my_steer_model(model, hf_model, sae, ft_id, hp, text, steer=None, percentage=5, batch_size=64, n_samples=128):

    final_layer_norm = hf_model.model.norm
    lm_head = hf_model.lm_head
    # decoder_weights = sae.W_dec
    # decoder_weights = final_layer_norm(decoder_weights[ft_id])
    # logits = lm_head(decoder_weights).unsqueeze(0)
    logits = lm_head(steer.unsqueeze(0))
    # print(logits.size())
    confidence = torch.softmax(logits, dim=1).detach()
    sae_topk = torch.topk(confidence, dim=1, k=100)
    # print(model.to_string(sae_topk.indices[:100]))
    # sys.exit()
    processors = LogitsProcessorList([SteeringLogitsProcessor(alpha=percentage/7.5, steering_vector=confidence)])
    toks = model.to_tokens(text, prepend_bos=True)
    toks = toks.expand(batch_size, -1)
    all_gen = []
    for _ in range(0, n_samples, batch_size):
        output_ids = hf_model.generate(toks, max_new_tokens=30, top_k=50, top_p=0.3, do_sample=True, logits_processor=processors)
        # print(model.to_string(output_ids))
        ...
        # print(gen_toks.size())
        # sys.exit()
        all_gen.extend(model.to_string(output_ids))
    return all_gen


# def my_steer_model(model, sae, ft_id, hp, text, percentage=5, batch_size=64, n_samples=128):
#     toks = model.to_tokens(text, prepend_bos=True)
#     toks = toks.expand(batch_size, -1)
#     all_gen = []
#     for _ in range(0, n_samples, batch_size):
#         with model.hooks([(hp, partial(my_patch_resid, sae=sae, ft_id=ft_id, percentage=percentage))]):
#             gen_toks = model.generate(
#                 toks,
#                 max_new_tokens=30,
#                 use_past_kv_cache=True,
